...
class Daemon {
	...
	public function __construct($is_sington = false, $output = "/dev/null") {
	...
	public function daemonize() {  
		...
		if ($this->is_sington == true) {  
			$this->pid_file = DAEMONTMPPATH . "/" .__CLASS__ . "_" . substr(basename($argv[0]), 0, -4) . ".pid";
			$this->checkPidfile();
		}
		...
		$this->setUser(USER) or die("cannot change owner");  
	...
	public function signalHandler($signo) {
		switch($signo) {
			//用户自定义信号
			case SIGUSR1: //busy
			if ($this->workers_count < DAEMONWORKERMAX){
				$pid = pcntl_fork();
				if ($pid > 0){
					$this->workers_count ++;
				}
	...
	*/  
	public function start($count = 1) {
		...
		while (true) {
			...
			if ($pid < 0) {
				$this->_log("daemon process create fork error ");  
				sleep(2);
			} else if ($pid == 0) {
				// 这个符号表示恢复系统对信号的默认处理  
				...
				do {
					$func_return = call_user_func($this->jobs['function']);  
					if ($func_return) {
						sleep(1);
					} else {
						sleep(30);
